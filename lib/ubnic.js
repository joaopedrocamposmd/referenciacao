...
(function () {
  ...
  function renderField(field, uid) {
    ...
    if (field.type === 'single') {
      ...
      if (dens === 'segmented') {
        ...
        inner = '<div class="u-radio' + (dens === 'buttons' ? ' u-btns' : '') + '"' + dn + def + '>' +
          (field.options||[]).map(o => '<label class="u-opt"><input type="radio" name="' + esc(fid) + '" value="' + esc(o.value) + '"><span class="u-radio-dot"></span><span class="u-opt-txt"><span class="u-opt-l">' + esc(o.label) + '</span>' + (o.desc ? '<span class="u-opt-d">' + esc(o.desc) + '</span>' : '') + '</span></label>').join('') + '</div>';
      }
    ...
    const label = (field.type === 'numeric' || field.type === 'single' || field.type === 'multi' || field.type === 'imc' || field.type === 'onset' || field.type === 'episodes' || field.type === 'berger' || field.type === 'decision')
  ...
  function isProgConfirm(field) {
    return field.type === 'multi' || field.type === 'numeric' || field.type === 'imc'
  ...
  UBNIC.stack = function (target, opts) {
    ...
    inst._onInput = (e) => {
      const t = e.target;
      if (t.matches && t.matches('.u-num') && !t.matches('[data-hard-min],[data-hard-max],[data-soft-min],[data-soft-max],[data-analyte],[data-sex]')) {
    ...
    inst._onStepBlur = (e) => {
      if (!inst._progressive) return;
      const t = e.target; if (!t.matches || !t.matches('.u-num')) return;   // só o input numérico standalone
      ...
      if (!field || field.type !== 'numeric') return;                        // compostos usam Confirmar
      if (inst.state[fid] == null || step.querySelector('.has-error')) return; // só com valor válido
      progMarkDone(inst, fid);
    };
  ...
  function recomputeState(inst) {
    ...
    for (const k in inst.state) { if (k.indexOf('_collapsed_') === 0 || k.indexOf('_normal_') === 0 || k.indexOf('_done_') === 0) keep[k] = inst.state[k]; }
    inst.state = keep;
  ...
  function buildResult(inst) {
    ...
    const pub = {}; for (const k in inst.state) { if (k.indexOf('_collapsed_') === 0 || k.indexOf('_done_') === 0) continue; pub[k] = inst.state[k]; }
  ...
  function syncProgressive(inst) {
    ...
    for (const f of fields) {
      ...
      if (inst.state['_done_' + f.id] === true) {
        ...
        if (sum) { const t = typeof f.summary === 'function' ? f.summary(inst.state[f.id], inst.state) : labelForValue(f, inst.state[f.id]); sum.textContent = t || ''; }
      } else if (f.id === curFid) {
  ...
  function markProgHydrated(inst) {
    if (!inst._progressive) return;
    inst.modules.forEach(m => (m.fields || []).forEach(f => { if (f.type !== 'separator' && inst.state[f.id] !== undefined) inst.state['_done_' + f.id] = true; }));
  }
  ...
  function destroyStack(inst) {
    ...
    inst._onChange = inst._onInput = inst._onHead = inst._onSec = inst._onSecChange = inst._onStep = inst._onStepBlur = null; // liberta closures; idempotência explícita
